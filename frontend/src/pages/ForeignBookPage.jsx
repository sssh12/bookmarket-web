...
export default function ForeignBookPage() {
    ...
    api
      ...
      .then((response) => {
        if (Array.isArray(response.data)) {
          // 해외 도서만 필터링
          const foreignBooks = response.data.filter(
            (book) => book.origin === "FOREIGN",
          );
          setBooks(foreignBooks);
        } else {
  ...
  // [기능 추가] 검색어 또는 카테고리가 변경될 때 페이지를 1로 리셋
